/**
 * This file is should be included by a Relying Party (RP) that wishes for
 * their users to be able to authenticate using the Tor Project's IdP.
 * 
 * The IdP server which the RP sends their users to is hardcoded, here it has
 * been set to the IP address of the Tor Project IdP (93.95.228.166).  As
 * such, for a RP to send its users to different IdPs, the RP would need to
 * dynamically source different `include.js` files, depending on the IdP
 ...
 * sites currently on the internet are hardcoded to send users to either
 * https://login.persona.org, https://login.anosrep.org, or
 * https://login.dev.anosrep.org.
 *
 * This file was taken on 21 October 2014 from the Mozilla Persona staging
 * server, and the uncompressed, original source can be found at
 * https://login.anosrep.org/include.orig.js
 *
 ...
 * file, You can obtain one at http://mozilla.org/MPL/2.0/. */
  ...
  if (!navigator.id.request || navigator.id._shimmed) {
    var ipServer = "https://93.95.228.166";
    var userAgent = navigator.userAgent;
    // We must check for both XUL and Java versions of Fennec.  Both have
    // distinct UA strings.
